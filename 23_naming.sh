...
OUTPUT=$(rubocop --only ConstantName,VariableName,MethodName -D $REPO/*.rb)
...
if [ $? -eq 0 ]; then
    ...
    rubocop --only ConstantName,VariableName,MethodName -D $REPO/*.rb > /home/codio/workspace/autograding_logs/23_naming.log
    exit 1
fi
